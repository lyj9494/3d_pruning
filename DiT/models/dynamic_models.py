...
class DiT(nn.Module):
    ...
    def load_ranking(self, path, num_steps, timestep_map, thres):
        ...
        for idx, router in enumerate(routers):
            ...
            if idx % 3 != 0: # 250steps
                ...
                # if idx == 0:
                #     final_list = [i for i in range(self.depth*2)]
                #     self.rank[idx] = torch.tensor(final_list)
                #print(router(), STE.apply(router(), thres).nonzero())
                total_layer += 2 * self.depth
                # self.rank[idx] = finetune_mask(self.rank[idx])
    ...
    def forward(self, x, t, y, ori=False):
        """
        Forward pass of DiT.
        ...
        if not ori:
            ...
            if self.cur_timestep % 3 == 0: # 250steps
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale):
        ...
        #     'ori': False
        # }
        # model_out, macs, nparams = count_ops_and_params(self, example_inputs=example_inputs, layer_wise=False)
        # self.total_macs += macs
        # self.cur_timestep -= 1
        # print("#MACs: {:.4f} G".format(self.total_macs/1e9))  
...
def finetune_mask(mask):
    if len(torch.where(torch.eq(mask, 55))[0]) > 0: #or len(torch.where(torch.eq(mask, 54))[0]) > 0
        new_mask = mask
    else:
        new_mask = torch.tensor([], dtype=torch.int64)
