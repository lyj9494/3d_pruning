...
class GaussianDiffusion:
    ...
    def router_training_losses(self, model, *args, **kwargs):
        return self.training_losses(model, *args, **kwargs)
    ...
    def mask_training_losses(self, model, x_t, ori_xt, t, model_kwargs=None):
        assert model_kwargs is not None
        # 对模型推理过程建模，无需dropout
        # 原代码
        # model_kwargs['model_train'] = False # model_train为False，模型强制执行奇数时间步
        
        with th.no_grad():
            model_kwargs['ori'] = True
            ori_x_next = self.ddim_sample(
                model, ori_xt, t, 
        ...
        if t[0].item() % 3 != 0: #  t[0].item() % 2 == 0
            ...
            with th.no_grad():
                model_kwargs['model_train'] = False
        ...
        terms['mse'] = mean_flat((ori_x_next - x_next) ** 2)
        terms['ori_x_next'] = ori_x_next
        terms['x_next'] = x_next.detach()
...
def main(args):
    ...
    for step in range(1000):
        ...
        y = torch.randint(0,1000,(n,)).to(device)
