...
def main(args):
    ...
    opts = torch.optim.AdamW(
        [param for name, param in model.named_parameters() if "routers" in name],  # only train router params
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in loader:
            ...
            loss_dict = diffusion.router_training_losses(model, x, ts, model_kwargs)
            data_loss = loss_dict["mse"].mean()
            l1_loss = loss_dict["l1_loss"].mean()

            #print(f"Rank: {rank}, t: {t}, data loss: {data_loss}. L1 loss: {l1_loss}")
            loss = data_loss + args.l1 * l1_loss
            opts.zero_grad()
            model.zero_grad()
            ...
            if train_steps % args.log_every == 0:
                ...
                for name, loss in [("loss", running_loss), ("data_loss", running_data_loss), ("l1_loss", running_l1_loss)]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results-dir", type=str, default="l2c_results")
